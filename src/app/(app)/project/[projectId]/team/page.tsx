...
const projectTeamMembers: TeamMember[] = [
  allTeamMembers[0],
  allTeamMembers[1],
  allTeamMembers[2],
  allTeamMembers[5],
...
const ProjectTeam = () => {
  ...
  const filteredMembers = projectTeamMembers.filter((member) => {
    const matchesSearch =
  ...
  const handleRemoveMember = () => {
    if (memberToRemove) {
      toast({
        ...
        description: `${memberToRemove.name} has been removed from the project${unassignTasksOnRemove ? " and all tasks have been unassigned" : ""}`,
      });
  ...
  const handleChangeRole = () => {
    if (memberToChangeRole && newRole) {
      toast({
        title: "Role updated",
        description: `${memberToChangeRole.name}'s role has been changed to ${newRole}`,
      });
            ...
            <div className="space-y-4 pt-4">
              <Popover open={commandOpen} onOpenChange={setCommandOpen}>
                <PopoverTrigger asChild>
                  ...
                  >
                    {selectedMember
                      ? allTeamMembers.find((m) => m.id === selectedMember)
                          ?.name
                      : "Select team member..."}
                    ...
                    <CommandList>
                      ...
                      <CommandGroup>
                        {availableMembers.map((member) => (
                          ...
                          >
                            <Check
                              className={`mr-2 h-4 w-4 ${selectedMember === member.id ? "opacity-100" : "opacity-0"}`}
                            />
